...
column CA_MOYEN Format 99999
SET pagesize 1000
SELECT annee PARTITION BY annee,  c.CL_R c, p.CATEGORY ca, AVG(QTE*PU) AS CA_MOYEN
FROM VENTES v, TEMPS t, PRODUITS p, CLIENTS c
WHERE c.cl_id = v.cid
AND v.pid = p.pid
AND v.tid = t.tid
...
/* Question 2.3 */
SELECT RANK() OVER(PARTITION BY annee AND CL_NAME ORDER BY QTE DESC), t.annee, p.CATEGORY, p.PNAME
FROM VENTES v, TEMPS t, PRODUITS p, CLIENTS c
WHERE c.cl_id = v.cid
AND v.pid = p.pid
AND v.tid = t.tid;
